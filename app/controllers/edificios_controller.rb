class EdificiosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @edificio.save
        ...
        format.json { render action: 'show', status: :created, location: @edificio }
      else
  ...
  def destroy
    @edificio.destroy
    respond_to do |format|
      format.html { redirect_to edificios_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def edificio_params
      params.require(:edificio).permit(:calle, :altura, :cant_uh, :lat, :lon)
    end
end
